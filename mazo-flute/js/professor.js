...
async function loadProfessorPage() {
  document.getElementById('navBar').innerHTML =
    `<button class="nav-tab active">🎓 Painel do Professor</button>`;

  document.getElementById('pagesContainer').innerHTML = `
    <div id="page-professor" class="page active">
      ...
      <div class="cbox">
        <div class="add-student-form">
          ...
          <div class="add-field">
            <label>Senha</label>
            <input type="text" id="newPass" placeholder="ex: flauta2025">
          </div>
          <div class="add-field">
        ...
        <div class="tip" style="margin-top:1rem">
          🔑 Defina uma senha para cada aluno e compartilhe com ele após o cadastro.
        </div>
      </div>
...
async function refreshProfessorPage() {
  ...
  const pendingN = Object.keys(requests).length;
  ...
  document.getElementById('profStats').innerHTML = `
    ...
    <div class="prof-stat" style="${pendingN>0?'border:2px solid var(--gold)':''}">
      <div class="prof-stat-num" style="color:${pendingN>0?'var(--warn)':'var(--muted)'}">${pendingN}</div>
  ...
  if (pendingN > 0) {
      ...
      <div class="request-card">
        <div class="request-info">
          ...
          <div class="request-detail">
            ...
            para <strong>${next?.emoji} ${next?.name}</strong>
          </div>
        </div>
    ...
    reqEl.innerHTML = `
      ...
      <div class="requests-list">${cards}</div>`;
  } else {
    reqEl.innerHTML = '';
  }
  ...
  tbody.innerHTML = students.map((s, i) => {
    ...
    const opts = MONTHS_DATA.map((mo, j) =>
      `<option value="${j}" ${j===mIdx?'selected':''}>${mo.name} — ${mo.tag}</option>`
    ).join('');
    return `
    <tr>
      <td>
        <div class="student-name">${hasPending?'⏳ ':''}${s.name}</div>
        <div class="student-email">${s.email}</div>
      </td>
      <td>
        <span id="pw-${i}" style="font-size:.82rem;color:var(--muted)">${'•'.repeat(s.password?.length||6)}</span>
        <button onclick="togglePw(${i},'${s.email}')"
          style="background:none;border:none;cursor:pointer;font-size:.75rem;color:var(--gold3);font-family:var(--ff-body);margin-left:6px">ver</button>
      </td>
      <td><span class="badge badge--gold">${m.emoji} ${m.name}</span></td>
      <td>
        <select class="month-select" id="sel-${i}">${opts}</select>
      </td>
      <td style="display:flex;gap:6px;flex-wrap:wrap;padding:8px">
        <button class="save-row-btn" id="savebtn-${i}"
          onclick="saveMonth(${i},'${s.email}')">Salvar</button>
        <button class="save-row-btn" style="background:rgba(192,57,43,0.1);color:var(--danger)"
          onclick="removeStudent('${s.email}')">Remover</button>
      </td>
    </tr>`;
  }).join('');
}
...
async function saveMonth(rowIdx, email) {
  ...
  await dbSaveStudent(email, { monthIdx: newIdx });
...
async function approveRequest(email, currentMIdx) {
  ...
  await dbSaveStudent(email, { monthIdx: next });
  await dbClearRequest(email);
  await refreshProfessorPage();
}
